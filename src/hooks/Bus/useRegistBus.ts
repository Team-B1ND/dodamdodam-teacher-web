...
import { BusBasicInfoType } from 'types/Bus/bus.type'
...
export const useRegistBus = () => {
  const [busData, setBusData] = useState<
    Pick<
      BusBasicInfoType,
      'busName' | 'description' | 'leaveTime' | 'peopleLimit' | 'timeRequired'
    >
  >({
    busName: '',
    description: '',
    leaveTime: dateTransform.hyphen(),
    peopleLimit: 0,
    timeRequired: '',
  })
  ...
  const handleLeaveDataDate = useCallback((e: Date) => {
    setBusData((prev) => ({
      ...prev,
      leaveTime: dayjs(e).format('YYYY-MM-DD'),
    }))
  }, [])
  ...
  const submitRegistBus = () => {
    registerBusMutatation.mutate(busData, {
      onSuccess: () => {
